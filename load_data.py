...
def fetch_electrical_parts() -> dict[str, ElectricalPart]:
    electrical_parts_data = pd.read_csv(os.path.join(os.getenv('ELECTRICAL_PARTS_FOLDER_PATH', 'Electrical Parts Report Modified.csv')))

    electrical_parts = dict()
    for _, electrical_part in electrical_parts_data.iterrows():
        part_id = electrical_part['PART_ID_CLEANED']

        electrical_parts[part_id] = ElectricalPart(
            part_id,
